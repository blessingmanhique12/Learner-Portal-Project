...
onAuthStateChanged(auth, async (user) => {
	...
	try {
		...
		try {
			...
			console.warn('Could not read Firestore profile. Using stored facilitator session.', error);
		}

		if (profile?.role && profile.role !== 'facilitator') {
			await signOut(auth);
			redirectToLogin();
			return;
		}
		...
		roleValue.textContent = profile?.role || 'facilitator';
		...
		setMessage(error.message || 'Could not load your facilitator profile.', 'error');
	}
});
